...
class ResNet_locate(nn.Module):
    def __init__(self, block, layers):
        ...
        self.mid_channels=[128,64,32,16,8]
        #change channels
        self.ppms_pre = nn.Conv2d(2048, self.in_planes, 1, 1, bias=False)
        ppms, infos = [], []
        cnt=0
#         for ii in [1, 3, 5, 7, 9]:
#             ppms.append(nn.Sequential(nn.AdaptiveAvgPool2d(ii), nn.Conv2d(self.in_planes, self.mid_channels[cnt], 1, 1, bias=False), nn.ReLU(inplace=True)))
#             cnt+=1
#         self.ppms = nn.ModuleList(ppms)

#         self.ppm_cat = nn.Sequential(nn.Conv2d(sum(self.mid_channels)+self.in_planes, self.in_planes, 3, 1, 1, bias=False), nn.ReLU(inplace=True))
