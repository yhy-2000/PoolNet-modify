...
class vgg16_locate(nn.Module):
    def __init__(self):
        ...
        self.vgg16 = vgg16()
        self.in_planes = 512
        self.ppm_out_planes=32
        self.out_planes = [512, 256, 128]

        ppms, infos = [], []
        for ii in [1, 3, 5]:
            ppms.append(nn.Sequential(nn.AdaptiveAvgPool2d(ii), nn.Conv2d(self.in_planes, self.ppm_out_planes, 1, 1, bias=False), nn.ReLU(inplace=True)))
        self.ppms = nn.ModuleList(ppms)
        self.info_midchannels=32
        self.ppm_cat = nn.Sequential(nn.Conv2d(self.ppm_out_planes * 3+self.in_planes, self.in_planes, 3, 1, 1, bias=False), nn.ReLU(inplace=True))
        for ii in self.out_planes:
            infos.append(nn.Sequential(nn.Conv2d(self.in_planes, self.info_midchannels, 3, 1, 1, bias=False),nn.Conv2d(self.info_midchannels,ii,1), nn.ReLU(inplace=True)))
    ...
    def forward(self, x):
        ...
        #xs is the several vertical line point to F
        #info is GGM
        return xs, infos
# a=vgg16_locate()
# print(a)